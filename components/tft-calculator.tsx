...
const TFTCalculator = () => {
  ...
  const handleCalculate = () => {
    ...
    if (
      !isNaN(parsedGold) &&
      parsedGold >= 2 &&
      !isNaN(parsedUnitsOut) &&
      !isNaN(parsedTotalUnitsOut)
    ) {
      ...
      const singleShopChance = 1 - Math.pow(1 - unitChance, 5);
      const result = 1 - Math.pow(1 - singleShopChance, rolls);
    ...
    <div className="flex justify-center items-center h-screen">
      <Card className="max-w-md w-full p-5 shadow-lg">
        ...
        <CardContent className="grid gap-4 mb-4 w-full max-w-md">
            ...
            <SelectContent>
              {TIERS.map((tier) => (
                <SelectItem
                  key={tier}
                  value={tier.toString()}
                  style={{ color: TIER_COLORS[tier], fontWeight: 500 }}
                >
                  Tier {tier}
                </SelectItem>
              ))}
          ...
          <div className="mb-4">
            <Label htmlFor="units-out">
              Total units of desired champion already taken
            </Label>
            <Input
          ...
          <div className="mb-4">
            <Label htmlFor="total-units-out">
              Total {tier ? tier : "X"} cost units out of the pool
            </Label>
            <Input
